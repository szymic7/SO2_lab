...
find 'coreutils-8.32/' -type f | xargs basename --multiple | sort | uniq --count | sort --numeric-sort --reverse | tr -s ' ' | head -n 1 | cut --delimiter ' ' --fields=3

# uniq --count -> poprzedza kazdy wiersz liczba wystapien (wartosci liczbowe wyrownnane do prawej)