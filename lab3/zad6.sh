...
if [ -d dane/pierwiastki ]; then
    for file in dane/pierwiastki/*; do
        if [ -h "${file}" ]; then # sprawdzenie czy plik jest dowiazaniem miekkim
            link="$(readlink "${file}")" # pobranie sciezki do pliku wskazywanego przez dowiazanie
            if [[ "${link}" = /* ]]; then # sprawdzenie czy sciezka jest bezwzgledna
                echo "$(realpath --canonicalize-missing "${link}")"
            fi
        fi
    done
fi