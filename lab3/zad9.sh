...
if [ -d dane ]; then
    for file in dane/* dane/*/*; do
        if [ -h "${file}" ] && [ ! -e "$(readlink -f "${file}")" ]; then # sprawdzenie, czy dowiazanie wiszace
            for target_file in dane/* dane/*/*; do
                if [ -f "${target_file}" ] && [ "$(basename "${target_file}")" = "$(basename "${file}")" ]; then
                    echo "$(basename "${file}"): $(realpath --relative-to="$(dirname "${file}")" "${target_file}")"
                    break
                fi
            done
        fi
